...
const UpdateFileAdmin = () => {
  ...
  const API_BASE = "http://localhost:8000/folder";
  ...
  useEffect(() => {
    const fetchFile = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/files/${id}/update`, {
          headers: {
        ...
        setFormData({ name: data.name, file_zip: null }); // file_zip null initially
        setLoading(false);
      } catch (err) {
        setMessage(err.message);
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/update_file/${id}/`, {
      ...
      if (!res.ok) throw new Error(`Failed to update file (status: ${res.status})`);
  ...
  return (
    <div className="admin-main-bg">
      ...
      <div className="update-file-admin admin-main-div">
        <h2>Update File</h2>
        {message && <p>{message}</p>}
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <div className="row">
            <label>Upload New ZIP (optional):</label><br />
            <input type="file" name="file_zip" accept=".zip" onChange={handleChange} />
          </div>
